    ...
    while (true) {
...

      {
        ...
        if (CPUThreadTime > 0) {
          ...
          System.out.println(
              CPULOADdata + "% usage" + "    " + Nano1 + "  " + Nano2 + "  Thread  " + ThreadTime1 + "  "
                  + ThreadTime2);
          double CPULOAD = (long) CPULOADdata;

          try {
            try {
              String fileCreatedStatus = conf.CPUfile();
              if (!fileCreatedStatus.isEmpty() || fileCreatedStatus == "File already exists.") {
                System.out.println(fileCreatedStatus);
              }
            } catch (IOException e) {
